...
namespace Demo.DobeCat.Sys.Platform.Windows
{
    ...
    /// <para>
    /// 仅适用于 <b>Built-in 渲染管线</b>。URP 下需改用 Renderer Feature；HDRP 不可用。
    /// </para>
    /// <para>必须在 PlayerSettings 里关闭
    /// <b>"Use DXGI Flip Model Swapchain for D3D11"</b>，否则 backbuffer 不带 alpha。</para>
    /// </summary>
    ...
    public class TransparentRenderBlit : MonoBehaviour
    {
        ...
        private void EnsureMaterial()
        {
            ...
            // 优先 Resources（保证 build 中也能加载）；找不到再退回 Shader.Find（仅 Editor / Always Included 可用）。
            ...
            if (sh == null)
            {
                Debug.LogError("[TransparentRenderBlit] 找不到 Shader 'Custom/MakeTransparent'。" +
                               "请确认 Assets/Demo/DobeCat/Resources/Shaders/MakeTransparent.shader 存在。");
                return;
            }
