    ...
    public class PetHUD : MonoBehaviour
    {
        ...
        private void TryTriggerHotbarTimer()
        {
            ...
            for (var i = 0; i < 9 && !triggered; i++)
                triggered = input != null && input.IsDown(InputManager.GetHotbarUseActionName(i));

            var scrollDelta = input != null ? input.GetMouseScroll() : Input.mouseScrollDelta.y;
            if (!triggered && Mathf.Abs(scrollDelta) > 0.01f) triggered = true;
        ...
        private void BuildQuickBar()
        {
            ...
            // AI 按钮：AI 模式用 Accent 高亮，手动模式用普通背景
