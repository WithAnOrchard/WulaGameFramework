...
namespace EssSystem.UIManager.Dao
{
    ...
    public abstract class UIComponent : Adjustable
    {
        ...
        /// </summary>
        [NonSerialized]
        private Dictionary<string, UIComponent> _children;
        ...
        {
            if (_children.TryGetValue(childId, out UIComponent child))
            {
        ...
        {
            return _children.TryGetValue(childId, out UIComponent child) ? child : null;
        }
        ...
        {
            UIComponent target = GetComponent(idPath);
            if (target != null && modifyAction != null)
            {
                modifyAction(target);
            }
            return this;
        ...
        public bool IsButton() => Type == UIType.Button;

        /// <summary>
        /// 检查是否为面板类型
        /// </summary>
        /// <returns>是否为面板类型</returns>
        public bool IsPanel() => Type == UIType.Panel;

        /// <summary>
        /// 检查是否为文本类型
        /// </summary>
        /// <returns>是否为文本类型</returns>
        public bool IsText() => Type == UIType.Text;
        ...
        protected virtual void OnNameChanged(string newName) => NotifyEntityPropertyChanged("Name", newName);
        protected virtual void OnVisibleChanged(bool visible) => NotifyEntityPropertyChanged("Visible", visible);
        protected virtual void OnInteractableChanged(bool interactable) => NotifyEntityPropertyChanged("Interactable", interactable);
        protected override void OnPositionChanged(Vector2 newPosition) { base.OnPositionChanged(newPosition); NotifyEntityPropertyChanged("Position", newPosition); }
        protected override void OnSizeChanged(Vector2 newSize) { base.OnSizeChanged(newSize); NotifyEntityPropertyChanged("Size", newSize); }
        protected override void OnScaleChanged(Vector2 newScale) { base.OnScaleChanged(newScale); NotifyEntityPropertyChanged("Scale", newScale); }
