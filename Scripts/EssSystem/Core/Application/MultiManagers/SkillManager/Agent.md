...
调用方式：
```csharp
CommonSkills.EnsureRegistered();             // 一次性注册除 Summon 外的所有默认技能
EventProcessor.Instance.TriggerEventMethod(  // 自定义参数版本（同 ID 后注册覆盖前注册）
    SkillManager.EVT_REGISTER_SKILL,
    new List<object> { CommonSkills.BuildFireball(damage: 30f) });
```
...
```csharp
ComboTracker.RegisterCombo(new ComboTracker.ComboDefinition {
    FinisherSkillId = "common_jump_slash",
    Sequence = new List<string> { "common_dash", "common_cleave", "common_dash" },
    WindowSeconds = 2.5f,
    ComboCooldown = 5f,
});
```
