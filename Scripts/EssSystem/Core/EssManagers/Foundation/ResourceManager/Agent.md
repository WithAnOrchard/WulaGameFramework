...
> **FBX 模型**：放进 `Resources/` 后，`Resources.Load<GameObject>(path)` 直接拿到模型根 ——
> 通过 `EVT_GET_PREFAB` 即可获取（FBX 的根资产类型就是 `GameObject`）。
> FBX 内部的 **AnimationClip 子资产** 会在启动时被按 `clip.name`（FBX 内 take 名）自动索引到缓存，
> 通过 `EVT_GET_ANIMATION_CLIP` 直接按 clip 名取，无需关心它属于哪个 FBX。
...
### `EVT_GET_ANIMATION_CLIP` — 同步获取 AnimationClip
