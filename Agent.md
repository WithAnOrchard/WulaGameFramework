...
- 通用框架模块引用的图片、音频、配置、预制体、材质、Shader 等资源不得放在 `Demo/` 或 `FrameworkResources/<DemoName>/` 私有目录下；只要被 Foundation、Core、通用 Manager、通用 Service 或工具链复用，就必须迁入 `FrameworkResources/Common`、`FrameworkResources/Sound`、`FrameworkResources/Config/Framework` 或对应通用资源目录，并由 Demo 显式声明依赖。Demo 私有资源只能被该 Demo 自身引用，不能反向绑定到通用框架能力上。
...
- 所有中文注释、文档、JSON 文案必须保持真实 UTF-8 中文；“UTF-8 合法”不等于“内容没有乱码”。
- 常见事故原因：用错误代码页读取 UTF-8 文本后，再用 UTF-8 写回，产生 `鏂`、`绋`、`鈥`、`銆`、`锛`、`闈`、`妯`、`鎶`、`鐨`、`脳`、`搂`、`乣` 等可显示但语义错误的字符；普通 UTF-8 校验无法发现这种内容级乱码。
- 修改含中文的源码、JSON、Agent.md 时优先使用 `apply_patch`；不要用 PowerShell `Get-Content` + `Set-Content`、Python here-string、未显式指定 UTF-8 的脚本整文件重写中文内容。
- 必须脚本化处理时，读取和写入都显式使用 UTF-8；中文文本优先使用 Unicode escape 或外部 UTF-8 文件输入，写回后用 `unicode_escape` 或乱码特征扫描确认。
- 改完中文注释、文档或配置文案后，除 `encoding_lint.ps1` 外，还要搜索上述 mojibake 特征串；发现乱码先修复再继续。
